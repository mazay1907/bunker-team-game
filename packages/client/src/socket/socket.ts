...
import { createConnectionHygiene } from './connectionHygiene.js';
...
// ── Connection hygiene (BUGFIX_HOST_DUPLICATE_JOIN) ─────────────────────────
// Implementation lives in connectionHygiene.ts (keeps this file under the
// project's 250-line file cap); bound here to this module's socket instance
// and setActiveRoomCode setter, then re-exported so call sites are unaffected.

const connectionHygiene = createConnectionHygiene(socket, setActiveRoomCode);
export const ensureConnectedForRoom = connectionHygiene.ensureConnectedForRoom;
export const __resetConnectionHygieneForTests = connectionHygiene.__resetConnectionHygieneForTests;
